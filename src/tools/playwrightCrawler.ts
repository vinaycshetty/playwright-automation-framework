  ...
  try {
    ...
    if (opts.username && opts.password) {
      ...
      for (const s of cookieSelectors) {
        try {
          await page.click(s, { timeout: 500 }).catch(() => {});
        } catch (e) {}
      }
    ...
    if (opts.autoNav) {
      try {
        const paymentsUrl = new URL("ui/PAYMENTS/managePayments", opts.url)
          .href;
      ...
      try {
        await page.click(`//li/div[text()="${paymentType}"]`).catch(() => {});
      } catch (e) {}
      try {
        await page
      ...
      try {
        ...
        console.log(
          "Could not click Continue button, may not be present on all flows - proceeding with crawl",
        );
      }
      ...
      let waitSelector: string | undefined =
        typeof opts.waitForSelector === "string" && opts.waitForSelector.trim()
          ? opts.waitForSelector.trim()
          : undefined;
      ...
      if (typeof waitSelector === "string" && waitSelector.length) {
        try {
          ...
          console.log(
            `[playwrightCrawler] waitForSelector satisfied after autoNav: ${waitSelector}`,
          );
        } catch (err) {
          console.log(
            "Warning: waitForSelector timed out after autoNav for",
            waitSelector,
          );
          if (opts.waitForUser) {
            ...
            try {
              ...
              console.log("Timeout after user wait; proceeding with crawl.");
            }
          }
        }
      } else {
    ...
    if (waitSelector) {
      try {
        ...
        console.log(
          `[playwrightCrawler] Initial waitForSelector satisfied: ${waitSelector}`,
        );
      } catch (e) {
        console.log(
          "Warning: initial waitForSelector timed out for",
          waitSelector,
          "- will attempt autoNav then wait again.",
        );
      }
    }
    ...
    try {
      elements = (await page.evaluate(
        ({ max, rootSel }: { max: number; rootSel: string | null }) => {
          ...
          const root = rootSel
            ? (document.querySelector(rootSel) as Element | null)
            : document;
          ...
          const nodeList = Array.from(
            scope.querySelectorAll(
              'a,input,select,textarea,button,label,name,div,span,[data-qa],[data-action],[role="button"],[role="link"]',
            ),
          );
          const out = nodeList.slice(0, max || 500).map((n) => {
            ...
            if (e.getAttribute) {
              dataQa =
                e.getAttribute("data-qa") || e.getAttribute("dataQa") || null;
              dataAction =
                e.getAttribute("data-action") ||
                e.getAttribute("dataAction") ||
                null;
            }
            // if the element itself doesn't have a testing hook, try its closest ancestor
            if (!dataQa) {
              const anc = (e.closest &&
                e.closest("[data-qa]")) as Element | null;
              if (anc && anc.getAttribute)
                dataQa =
                  anc.getAttribute("data-qa") ||
                  anc.getAttribute("dataQa") ||
                  null;
            }
            if (!dataAction) {
              const anc = (e.closest &&
                e.closest("[data-action]")) as Element | null;
              if (anc && anc.getAttribute)
                dataAction =
                  anc.getAttribute("data-action") ||
                  anc.getAttribute("dataAction") ||
                  null;
            }
            // include clickable detection: anchors with href, elements with onclick or role=button
            const onclick =
              (e.getAttribute && e.getAttribute("onclick")) || null;
            ...
            try {
              ...
              if (!labelText) {
                const closestLabel = (e.closest &&
                  e.closest("label")) as Element | null;
        ...
        { max: opts.maxElements || 1000, rootSel: opts.rootSelector || null },
      )) as any[];
    } catch (errEval) {
      ...
      try {
        ...
        // eslint-disable-next-line no-console
        console.error(
          "[playwrightCrawler] page content (first 2000 chars):\n",
          snap.slice(0, 2000),
        );
      } catch (snapErr) {
        // eslint-disable-next-line no-console
        console.error(
          "[playwrightCrawler] failed to capture page content",
          snapErr,
        );
      }
      await browser.close();
    ...
    for (const e of elements) {
      const key =
        e.dataQa || e.dataTestId || e.id || e.name || e.tag + "|" + e.text;
