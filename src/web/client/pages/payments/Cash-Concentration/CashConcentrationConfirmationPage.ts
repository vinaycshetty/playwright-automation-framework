...
export class CashConcentrationConfirmationPage {
  ...
  async getPaymentResults(): Promise<CashConcentrationResult> {
    const result: CashConcentrationResult = {
      ...
      valueDate: this.extractValue(await this.getText(this.valueDateSelector))?.split(":")[1]?.trim() || "", // In case both Value Date and Payment Date are present, take the first one (Value Date)
