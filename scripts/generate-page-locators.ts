...
async function main() {
  ...
  if (!argv.url) {
    ...
    const maxElements = argv.maxElements ? parseInt(String(argv.maxElements), 10) : 2000;
  ...
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
    ...
    const allowedTags = new Set(['input','select','textarea','button','a','label']);
    const allowedRoles = new Set(['button','textbox','combobox','link']);
    let top = topRaw.filter((e: any) => {
      const tag = (e.tag || '').toLowerCase();
      const role = (e.role || '').toLowerCase();
      const hasHook = e.dataQa || e.dataAction || e.id || e.name || e.label || e.href || e.onclick;
    ...
    const hasMeaningfulName = (e: any) => {
      const txt = String(e.label || e.text || e.id || e.name || e.dataQa || e.dataAction || '').trim();
      if (!txt) return false;
      // if hook is generic like 'button-12' or 'select2-chosen-32', consider it meaningless unless a label/text exists
      if ((e.dataQa && genericHookRe.test(e.dataQa)) || (e.dataAction && genericHookRe.test(e.dataAction))) {
        return Boolean(e.label && String(e.label).trim().length > 2) || Boolean(e.text && String(e.text).trim().length > 2);
      }
      return true;
    };
  ...
  for (const e of top) {
    ...
    const rawKey = (e.dataQa || e.dataAction || e.id || e.name || (e.tag + '_' + (e.text || '').slice(0,10)));
    ...
    else if (e.dataAction) selStr = `[data-action="${escapeAttr(e.dataAction)}"]`;
    ...
    else if (e.classes) selStr = `${e.tag}${'.' + String(e.classes).split(/\s+/).filter(Boolean).join('.')}`;
    ...
    if (selStr.startsWith('xpath=') && isBadXpath(e.xpath)) {
      ...
      const meaningful = Boolean(e.label && String(e.label).trim().length > 1) || Boolean(e.text && String(e.text).trim().length > 1) || e.dataQa || e.dataTestId || e.id || e.name;
    ...
    if (badXpath) {
      ...
      if (tag === 'input' || tag === 'textarea' || tag === 'select') {
        if (hasLabel) {
          ...
          const methodKey = toVarName(e.label || e.name || e.id || (e.tag + '_' + (e.text || '').slice(0,10)));
          methodLines.push(`  async set_${methodKey}(value: string) { await this.page.getByLabel("${label}").fill(value); }`);
        } else if (e.name) {
          const namev = escapeAttr(e.name);
          const methodKey = toVarName(e.name);
          methodLines.push(`  async set_${methodKey}(value: string) { await this.page.locator("[name='${namev}']").fill(value); }`);
        }
      } else {
        ...
        if (hasLabel || hasText) {
          ...
          const role = (e.role || '').toLowerCase();
          const methodKey = toVarName(e.label || e.text || e.id || e.name || (e.tag + '_' + (e.text || '').slice(0,10)));
          if (role === 'button' || tag === 'button') {
            methodLines.push(`  async click_${methodKey}() { await this.page.getByRole("button", { name: "${nameText}" }).click(); }`);
          } else if (role === 'link' || tag === 'a') {
            methodLines.push(`  async click_${methodKey}() { await this.page.getByRole("link", { name: "${nameText}" }).click(); }`);
          } else {
            methodLines.push(`  async click_${methodKey}() { await this.page.getByText("${nameText}").click(); }`);
          }
        }
      }
    ...
    const tag = (e.tag || '').toLowerCase();
    if (tag === 'input' || tag === 'textarea' || tag === 'select') {
      methodLines.push(`  async set_${fieldName}(value: string) { await this.page.fill(this.${fieldName}, value); }`);
    } else {
      methodLines.push(`  async click_${fieldName}() { await this.page.click(this.${fieldName}); }`);
    }
  }
